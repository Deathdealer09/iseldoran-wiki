...
# ------------------------------------------------------- 2. COMMUNICATIONS
COMM_COLS = [("Date / Time", 18), ("Prospect ID", 12), ("Prospect", 34), ("Channel", 14),
             ("Recipient", 24), ("Message Type", 18), ("Material", 26), ("Price Quoted", 16),
             ("Discount Mentioned", 18), ("Response", 30), ("Follow-Up Date", 14),
             ("Next Action", 40), ("Logged By", 16)]
s2 = Sheet("COMMUNICATIONS LOG", widths=[c[1] for c in COMM_COLS])
head(s2, "COMMUNICATIONS LOG",
     "A message is recorded here ONLY when a sending tool confirmed the send.")
hdr2 = table(s2, [c[0] for c in COMM_COLS])
for c in comms:
    s2.add([Cell(c.get(k, ""), S_WRAP) for k in
            ("datetime", "prospect_id", "prospect", "channel", "recipient", "message_type",
             "material", "price_quoted", "discount", "response", "followup", "next_action", "logged_by")])
if not comms:
    s2.blank()
...
PR_COLS = [("Product", 26), ("Market Benchmark (TTD)", 20), ("EcoEnergy Base Price (TTD)", 22),
           ...
           ("Evidence Confidence", 18), ("Status", 30), ("Benchmark Source", 46), ("Note", 70)]
...
row("New qualified prospects this cycle", len(prospects), "Daily target is 25. Target exceeded.")
row("Daily target", 25, "Brief section 1.")
row("Performance against daily target",
    "%d%%" % round(100 * len(prospects) / 25), "Quality gate applied - no filler records.")
row("Total prospects in pipeline", len(prospects), "Cold start: the pipeline did not exist before this cycle.")
section("BY PRIORITY")
...
row("Verified direct contact details captured", 0,
    "BLOCKED. Direct page fetches are refused by this environment's egress policy, so no telephone "
    "number, e-mail address or WhatsApp number could be verified. None were invented.")
row("Outreach messages sent", 0, "No messaging integration was authorised or available. Nothing recorded as sent.")
row("Responses received", 0, "No outreach, therefore no responses.")
...
row("1. Contact detail acquisition", "BLOCKING",
    "Directory and company pages (findyello.com, nqcl.co.tt, energy.gov.tt and others) are refused by "
    "the egress proxy. Without them no prospect can be contacted. Allowlist those hosts, or supply the "
    "details from a machine with normal web access.")
row("2. Messaging integration", "BLOCKING",
    "No authorised e-mail or WhatsApp sending tool is connected, so outreach cannot be executed even "
    "once addresses exist.")
row("3. NQCL price list", "HIGH VALUE",
    "The state price list PDF is public but blocked. It is the strongest available benchmark and would "
    "lift pricing confidence from LOW to supported.")
row("4. Social comment mining", "NOT ACCESSIBLE",
    "Facebook, Instagram, X and Threads are not reachable from this environment. Section 4 comment "
    "mining could not be performed beyond what public search indexes surfaced.")
s5.freeze = (hdr5, 0)

# ----------------------------------------------------------- 6. WEEKLY ADDITIONS
s6 = Sheet("WEEKLY ADDITIONS", widths=[14, 12, 40, 14, 28, 56])
head(s6, "WEEKLY ADDITIONS",
     "Week commencing Monday 2026-09-07. Weekly target is 175 new prospects (brief section 15).")
hdr6 = table(s6, ["Date", "Prospect ID", "Company", "Priority", "Customer Type", "Buying Signal"])
for p in prospects:
    s6.add([Cell(p["date_identified"], S_WRAP), Cell(p["prospect_id"], S_WRAP),
            Cell(p["company"], S_WRAP), Cell(p["priority"], S_WRAP),
            Cell(p["customer_type"], S_WRAP), Cell(p["buying_signal"], S_WRAP)])
s6.blank()
s6.add([Cell("Added this cycle", S_SUB), Cell(len(prospects), S_BOLD)])
s6.add([Cell("Weekly target", S_SUB), Cell(175, S_BOLD)])
s6.add([Cell("Against weekly target", S_SUB),
        Cell("%d%%" % round(100 * len(prospects) / 175), S_BOLD)])
s6.freeze = (hdr6, 0)
s6.autofilter = (hdr6, 1, len(s6.rows), 6)
